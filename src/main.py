...
import pandas  as pd
from config  import ULTIMOS_DATOS
...
#variable global para almacenar los ultimos datos importados 
...
def exp_excel(datos,logger):
    if not datos:
         logger.warning("No hay datos para visualizar")
         return
    try:
        df = pd.DataFrame(datos)
        nombre_archivo = f"ventas_eds_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        df.to_excel(nombre_archivo,index=False)
        logger.info(f"Archivo Excel generado exitosamente: {nombre_archivo}")
    except Exception as e:
        logger.error(f"Error al generar archivo Excel: {e}", exc_info=True)
          
def salir_aplicacion(ventana, logger):
...
def limpiar_text_area(text_area):
    """Limpia el área de texto del logger en la interfaz."""
...
def procesa_fac(logger):
    """Procesa las facturas disponibles en base de datos."""
...
def impdata_api(fec_ini, fec_fin, logger):
    """Consulta facturas en el API y las inserta en base de datos."""
    clean_data(2)
    ...
    for branch_id in branch_ids:
        try:
            ...
            if not response_json_eds or "data" not in response_json_eds:
                logger.warning("No se pudo obtener datos de ventas para la estación %s.", branch_id,)
                continue

            _data = response_json_eds.get("data", {}).get("fe_sales", [])
            if not _data:
                logger.warning("No hay datos de ventas para la estación %s.",branch_id,)
                continue
            ...
            logger.info("Procesamiento de ventas completado para la estación %s.", branch_id,)

        except KeyError as e:
            logger.error("Error al acceder a los datos de ventas: Clave faltante %s.", e,exc_info=True,)
        except Exception as e:
            logger.error("Error inesperado al procesar ventas en la estación %s: %s.",branch_id, e,exc_info=True,)
  


def reprocesos(logger):
    """Reprocesa datos ya cargados en base de datos."""
    clean_data(0)
    procesa_fac(logger)


def ventana_fechas(logger, text_area):
    """Abre ventana para selección de rango de fechas."""

    def on_submit():
        fecha_inicio = date_entry_inicio.get()
        fecha_final = date_entry_final.get()
        top.destroy()
        limpiar_text_area(text_area)
...
def crear_ventana_principal():
    """Crea la ventana principal con área de texto y menú."""
    ventana = tk.Tk()
    ...
    text_area = scrolledtext.ScrolledText(
        ventana, wrap=tk.WORD, state="disabled"
    )
...
def main():
    ...
    menu_procesos.add_command(
        label="1. Procesar Facturas",
        command=lambda: ventana_fechas(logger, text_area),
    )
    menu_procesos.add_command(
        label="2. Reprocesar Novedades",
        command=lambda: reprocesos(logger),
    )
    menu_procesos.add_command(
        label="3. Exportar facturas procesadas",
        command=lambda: exp_excel(ULTIMOS_DATOS,logger),
    )
    menu_procesos.add_command(
        label="4. Salir de la aplicación",
        command=lambda:  salir_aplicacion(ventana, logger),
    )
